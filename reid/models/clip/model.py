...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.query = 3
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(self.query , width))
        self.positional_embedding = nn.Parameter(scale * torch.randn(h_resolution*w_resolution + self.query, width))
    ...
    def forward(self, x: torch.Tensor):
        ...
        #print("x:", x.shape, self.positional_embedding.to(x.dtype).shape)
        if cv_emb != None:
            x = x + self.positional_embedding.to(x.dtype) + cv_emb
        else:
        ...
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        #print("class_embedding:", self.class_embedding.to(x.dtype).shape)
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], self.query , x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1) # shape = [*, grid ** 2 + 1, width]
        #print("class_x:", x.shape)
        # score_embedding
        ...
        x = x + self.positional_embedding.to(x.dtype) + x_
        x = self.ln_pre(x)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        #print("transformer_width", transformer_width, embed_dim)
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int):
    ...
    if vit:
        state_dict["visual.positional_embedding"] = resize_pos_embed(state_dict["visual.positional_embedding"], model.visual.positional_embedding, model.visual.class_embedding, h_resolution, w_resolution)
        state_dict["visual.class_embedding"] = resize_class_token(state_dict["visual.class_embedding"], model.visual.class_embedding)
    else: #RN50
...
'''def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    print("posemb_token", posemb_token.shape, posemb_grid.shape)
    ntok_new -= 1
...
def resize_pos_embed(posemb, posemb_new, class_embedding, hight, width):
    # Rescale the grid of position embeddings when loading from state_dict. Adapted from
    # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
    print('Resized position embedding: %s to %s', posemb.shape, posemb_new.shape)
    class_tokens = class_embedding.shape[0]
    ntok_new = posemb_new.shape[0]  # 129,2048
    posemb_token, posemb_grid = posemb_new[:class_tokens], posemb[1:]  #4,768   196,768
    ntok_new -= class_tokens
...
def resize_class_token(classs, class_new):
    # Rescale the grid of position embeddings when loading from state_dict. Adapted from
    # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
    class_token = class_new.shape[0]
    posemb_grid = classs.reshape(1, 1, 1, -1).permute(0, 3, 1, 2)
    posemb_grid = F.interpolate(posemb_grid, size=(1, class_token), mode='bilinear')
    posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(class_token, -1)
    #posemb = torch.cat([posemb_token, posemb_grid.squeeze()], dim=0)
    return posemb_grid
